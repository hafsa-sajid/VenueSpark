...
function VerifyOTP() {
  ...
  // Extract email passed from SignUp
  const email = location.state?.email || "";
  
  const [otp, setOtp] = useState("");
  const [loading, setLoading] = useState(false);

  // If there's no email in state, the user probably navigated here directly. Redirect to signup
  if (!email) {
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      if (result.status === 201 || result.status === 200) {
        toast.success("Account Verified Successfully!", { style: { height: "50px", width: "300px" } });
        setUserData(result.data); // Store user state
        navigate("/");
      }
    } catch (error) {
      setLoading(false);
  ...
  return (
    <div className='min-h-screen w-full flex bg-transparent transition-colors duration-500 overflow-hidden relative'>
      ...
      <div className='hidden lg:flex w-1/2 relative flex-col items-center justify-center p-12 bg-transparent border-r border-white/5 shadow-2xl'>
          ...
          <div className='relative z-10 flex flex-col items-center text-center animate-pop-3d'>
              ...
              <p className='text-slate-400 text-xl font-medium max-w-md leading-relaxed'>
                  One final step to join the elite. Secure your account and start exploring legendary spaces.
              </p>
          </div>
      ...
      <div className='w-full lg:w-1/2 flex items-center justify-center relative p-6 bg-transparent transition-colors duration-500'>
          ...
          <form className='glass-card max-w-[480px] w-full p-10 md:p-14 flex flex-col gap-8 relative' onSubmit={handleVerifyOTP}>
              <div className='w-full text-center'>
                ...
                <h1 className='text-4xl font-black text-white tracking-tighter leading-none'>Verify Email</h1>
                <p className='text-slate-500 text-sm font-medium mt-4 uppercase tracking-[2px]'>
                    Sent to <span className='text-[#8B5CF6] font-black'>{email}</span>
                </p>
              </div>
      
              <div className='w-full space-y-4'>
                  <label htmlFor="otp" className='text-[10px] font-black uppercase tracking-[3px] text-slate-500 text-center block'>Enter 6-Digit Code</label>
                  <input 
                    type="text" 
                    id='otp' 
                    className='w-full h-18 bg-white/5 border-2 border-white/5 rounded-2xl text-[28px] font-black text-center tracking-[0.5em] px-5 focus:outline-none focus:border-[#8B5CF6] focus:bg-white/10 text-white placeholder-slate-700 transition-all shadow-inner' 
                    maxLength="6"
              ...
              <p className='text-center text-sm font-bold text-slate-500 mt-4'>
                  Didn't receive the code? <span className='text-[#8B5CF6] font-black cursor-pointer hover:underline uppercase tracking-widest text-xs'>Resend Code</span>
              </p>
          </form>
      </div>
    </div>
