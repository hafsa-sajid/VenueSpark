    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (result.status === 200 || result.status === 201) {
               toast.success("Login Successfully.", { style: { height: "50px", width: "300px" } })
            }
    ...
    return (
        <div className='min-h-screen w-full flex bg-transparent transition-colors duration-500 overflow-hidden relative'>
            ...
            <div className='hidden lg:flex w-1/2 relative flex-col items-center justify-center p-12 bg-transparent border-r border-white/5 shadow-2xl'>
                ...
                <div className='relative z-10 flex flex-col items-center text-center animate-pop-3d'>
                    ...
                    <p className='text-slate-400 text-xl font-medium max-w-md leading-relaxed'>
                        Enter the portal to legendary event spaces. Your next masterpiece begins with a single spark.
                    </p>
                </div>
                
                <div className='absolute bottom-12 text-slate-500 text-[10px] font-black uppercase tracking-[6px]'>
                    Elite Event Management Console
                </div>
            </div>
            ...
            <div className='w-full lg:w-1/2 flex items-center justify-center relative p-6 bg-transparent transition-colors duration-500'>
                ...
                <form className='glass-card max-w-[480px] w-full p-10 md:p-14 flex flex-col gap-8 relative' onSubmit={handleLogin}>
                    <div className='w-full text-center'>
                      ...
                      <h1 className='text-4xl font-black text-white tracking-tighter leading-none'>Welcome Back</h1>
                      <p className='text-slate-500 text-sm font-medium mt-4 uppercase tracking-[2px]'>Log in to your account</p>
                    </div>
                    ...
                    <div className='w-full space-y-3 relative'>
                        <div className="flex justify-between items-center ml-1 pr-1">
                            <label htmlFor="password" className='text-[11px] font-black uppercase tracking-[3px] text-slate-300'>Password</label>
                            <span className="text-[11px] text-[#8B5CF6] cursor-pointer hover:underline font-black uppercase tracking-[2px]" onClick={() => navigate("/forgot-password")}>Forgotten?</span>
                        </div>
                    ...
                    <p className='text-center text-sm font-bold text-slate-500 mt-4'>
                        New to VenueSpark? <span className='text-[#8B5CF6] font-black cursor-pointer hover:underline uppercase tracking-widest text-xs' onClick={()=>navigate("/signup")}>Sign Up</span>
                    </p>
                </form>
            </div>
        </div>
